...
- `assets/js/mesh-field.js`: unica eccezione al JS inline, perché identico su tutte e otto le pagine. Foglio triangolato in WebGL vanilla su un `<canvas>` fisso a `z-index:-1`, invisibile a riposo, sollevato sotto il cursore. Niente simulazione: la quota è una formula chiusa (bump `PEAK` alto e `RADIUS` largo) valutata nel vertex shader da un solo punto, il puntatore filtrato con un esponenziale temporale (`POINTER_TAU` 85 ms), quindi a cursore fermo il frame successivo è identico e il rAF si ferma. Dalla quota derivano rilievo (prospettiva debole, scivolamento lungo la pendenza) e luce (alpha, spessore e colore continui, dal grigio al `TINT` ciano `#5fb0d6` fino a quasi bianco sulla cresta; `TINT_MIX 0` per un foglio grigio). Nella colonna di lettura (`--measure`) l'intensità è tenuta al 50% con rampa di 100 px. Parametri in testa al file; si spegne con `prefers-reduced-motion`, su puntatori grossolani, senza WebGL e su `<body data-mesh-field="off">`. Le quattro alternative scartate (`aurora`, `dots`, `flow`, `contours`) restano in `drafts/bg/` con la pagina di confronto `drafts/bg/preview.html?v=slug`; per cambiare basta copiare `drafts/bg/<slug>.js` su questo file.
