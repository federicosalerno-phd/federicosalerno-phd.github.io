...
- `assets/js/mesh-field.js`: unica eccezione al JS inline, perché è identico su tutte e otto le pagine. Reticolo triangolato su un `<canvas>` fisso a `z-index:-1`, invisibile a riposo: sotto il cursore i nodi vengono spinti fuori e gli spigoli si accendono in proporzione allo spostamento, poi rientrano con una molla smorzata. Parametri raccolti in testa al file (raggio, spinta, molla, smorzamento, opacità); si spegne da solo con `prefers-reduced-motion`, su puntatori grossolani e su `<body data-mesh-field="off">`; il rAF si ferma quando la mesh è ferma e il cursore è fuori. Ogni pagina lo carica con una riga prima di `</body>`.
