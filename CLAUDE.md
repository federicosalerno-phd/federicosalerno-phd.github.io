...
- `assets/js/mesh-field.js`: unica eccezione al JS inline, perché identico su tutte e otto le pagine. Reticolo triangolato in WebGL vanilla su un `<canvas>` fisso a `z-index:-1`. Il reticolo è sempre disegnato, a `REST_A` (circa cinque livelli sopra `#111` nei margini, metà dietro il testo). Il puntatore seleziona la propria faccia: ogni triangolo è riempito con il valore di una sola caduta liscia (`fall()`) presa al suo baricentro, quindi la faccia sotto il cursore è la più chiara, l'anello intorno un gradino sotto, e il bordo della macchia è fatto di lati del reticolo, mai un cerchio; ogni faccia ha inoltre una soglia propria (`JAG`), fissa e ricavata dal suo baricentro, che spezza la silhouette lungo il reticolo invece di lasciarla tonda. La faccia selezionata si gonfia attorno al proprio baricentro (`GROW`), e la stessa caduta accende spigoli e nodi e solleva il foglio di `PEAK` (9 px: la differenza fra un foglio che si piega e una bolla che scivola). Niente simulazione: formula chiusa della posizione del puntatore, che non è filtrata (`POINTER_TAU 0`: nessuna inerzia, la faccia sta sotto il cursore e non dietro), quindi a cursore fermo il frame successivo è identico e il rAF si ferma; tre draw call su buffer statici, nessun lavoro per nodo sulla CPU. Colore `TINT` ciano `#5fb0d6` (`TINT_MIX 0` per un reticolo grigio). Colonna di lettura (`--measure`) al 50% con rampa di 100 px. Si spegne con `prefers-reduced-motion`, su puntatori grossolani, senza WebGL e su `<body data-mesh-field="off">`. Le quattro alternative scartate (`aurora`, `dots`, `flow`, `contours`) restano in `drafts/bg/` con la pagina di confronto `drafts/bg/preview.html?v=slug`; per cambiare basta copiare `drafts/bg/<slug>.js` su questo file.
