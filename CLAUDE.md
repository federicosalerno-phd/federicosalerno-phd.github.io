...
- `assets/css/strike.css` e `assets/js/strike.js`: l'hover di tutto il sito, seconda e terza eccezione alla
  regola dell'inline, per lo stesso motivo di `mesh-field.js` (identici su tutte e otto le pagine). Il problema che
  risolvono: un `:hover` con transizione appartiene al puntatore, quindi attraversare una banda in 80 ms lasciava
  il movimento a metà e lo rimandava indietro — gli scatti scendendo l'elenco. Qui il movimento non è più uno
  stato: al passaggio del bordo la superficie viene **colpita**, e l'impulso sono due `@keyframes` avviate da una
  classe e tolte dal proprio `animationend`, quindi arrivano in fondo anche se il puntatore è già tre bande più
  giù. Le due metà sono la dimensione (su `scale`, non `transform`, così compone con `idx-rise` e con le scale già
  dichiarate) e la luce: il riempimento si apre dal punto esatto del contatto in tre gradini piatti, non su una
  rampa, perché è così che il reticolo sfuma (`STEPS`); sulle righe della home i gradini sono il velo di `--btn`
  che viene tolto e per un istante la banda è quasi un buco con il reticolo intero che la attraversa. Il passaggio
  allo stato tenuto è gratis: l'ultimo keyframe vale esattamente quello che dichiara la regola `:hover`, quindi se
  il puntatore è rimasto non si muove nulla, se è andato via parte il rilascio da dove l'impulso era arrivato.
  `strike.js` è delegato da `document` (nessun listener per elemento, niente per frame), scrive `--kx`/`--ky` (il
  punto d'ingresso, per cui due passaggi non sono mai uguali), mette la classe, e non fa nulla se il `<body>` non
  dice `data-hover="strike"` o con `prefers-reduced-motion`. Il foglio è linkato DOPO lo `<style>` inline di ogni
  pagina: è quello che gli permette di scavalcare le regole di hover scritte lì senza selettori più pesanti.
  Famiglie e numeri (tempo/ampiezza/tenuta/press) in testa al file: bande `.row .back .pdf` 520 ms, card
  `.tile .paper` 460, miniature `.gallery li` 520, pillole e bottoni dei visualizzatori 340-420, `.avatar-btn` 400.
  Quattro stati nel `<body>` di ogni pagina, tutti ancora nel repo: `strike` (attuale), `spring` (la molla che
  solleva la banda), `wipe` (il riempimento che scorre) e nessun attributo (l'hover originale a scambio di colore).
